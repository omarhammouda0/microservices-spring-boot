...
- **Inter-service communication** via OpenFeign + Eureka load-balancing.
- **Circuit breaker + retry** (Resilience4j) on Feign calls with graceful fallback to cached data.
- **Idempotent event processing** via a `processed_events` table.
- **Soft delete** for users and products (`isActive` flag).
- **Order state machine** with validated transitions (`PLACED → CONFIRMED → SHIPPED → DELIVERED`, plus `CANCELLED`).
- **Optimistic locking** (`@Version`) with `@Retryable` on inventory updates.

### Event-Driven Flows (RabbitMQ)
- `order.created` — published by `order-service`, consumed by `product-service` to deduct inventory.
- `order.cancelled` — published by `order-service`, consumed by `product-service` to restore inventory.
- `user.updated` — published by `user-service` on update / soft-delete, consumed by `product-service` to invalidate Redis user cache.
- All queues have **dead-letter exchanges** (`order.dlx`) for failed messages.
